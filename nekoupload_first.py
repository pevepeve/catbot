...
async def upload_media_files(folder, method):
    ...
    for filename in os.listdir(folder_path):
        ...
        try:
            exists = session.query(NekoImage.filename).filter_by(filename=filename).first() is not None
        finally:
            session.close()
...
wait_tasks = asyncio.wait([loop.create_task(upload_media_files("nekochans", bot.send_photo))])
